var SharedParserConfig = Object.create({
      htmlsplitters: [
        {
          splitters: ['<img ', '<source ', '<script '],
          rgx: new RegExp(/(?:src)=['"](?!\w+?:?\/\/)([^'"\{]+)['"].*\/?>/i)
        },
        {
          splitters: ['<link '],
          rgx: new RegExp(/(?:href)=['"](?!\w+?:?\/\/)([^'"\{]+)['"].*\/?>/i)
        },
        {
          splitters: ['<script '],
          rgx: new RegExp(/data-main=['"](?!\w+?:?\/\/)([^'"\{]+)['"].*\/?>/i)
        }
      ],
      regcss: new RegExp(/url\(([^)]+)\)/ig),

      regcssfilter: new RegExp(/filter[\w\.\:]+\(src=['"]([^'"]+)['"]/ig),

      supportedTypes: {
        html: 'html',
        css: 'css',
        soy: 'html',
        ejs: 'html',
        hbs: 'html'
      }
    });

exports.css = function(options) {
  var CSSJob = require('./css_job');
  return new CSSJob(options);
};

exports.html = function(options) {
  var HTMLJob = require('./html_job');
  return new HTMLJob(options);
};

exports.registerParser = function(type, config) {
  SharedParserConfig[type].push(config);
};

exports.SharedParserConfig = SharedParserConfig;